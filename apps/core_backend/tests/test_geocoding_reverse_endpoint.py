...
from schemas.geocoding import GeocodingReverseResponseModel
...
class _FakeGeocodingAdaptor:
    ...
    async def reverse(self, _client, request):
        ...
        return GeocodingReverseResponseModel(
            ...
            results=[
                Place(
                    ...
                    address="Main Street 1, City, 12345",
                    street="Main Street",
                    locality="City",
                    postcode="12345",
...
def test_geocoding_reverse_success(monkeypatch):
    ...
    assert fake_adaptor.last_request.lat == 49.44
    assert fake_adaptor.last_request.lon == 7.75
    assert fake_adaptor.last_request.limit == 3


def test_geocoding_reverse_default_limit(monkeypatch):
    fake_adaptor = _FakeGeocodingAdaptor()
    monkeypatch.setattr(geocoding_endpoint, "adaptor", fake_adaptor)
    client = TestClient(app)
    ...
    assert response.status_code == 200
    assert fake_adaptor.last_request.limit == 5
